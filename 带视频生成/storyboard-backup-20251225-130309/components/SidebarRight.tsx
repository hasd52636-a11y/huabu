...
const SidebarRight: React.FC<SidebarRightProps> = ({ 
  lang, theme, isLoading, isExpanded, setIsExpanded, onGenerateFromScript, onExportPrompts, onExportJPEG, getFormattedPrompts, model, setModel, onGenerateFromDialogue, globalColorMode, onOpenHelp, onStyleChange, onAspectRatioChange, onGenerateVideo
}) => {
              ...
              <div className="h-full flex flex-col gap-0 overflow-hidden">
                ...
                <div className="mx-4 mb-4 flex space-x-3">
                  <button 
                    ...
                    className="flex-1 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black uppercase tracking-widest rounded-2xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 text-sm flex-shrink-0"
                  >
                  ...
                  <button 
                    ...
                    title={lang === 'zh' ? '生成视频' : 'Generate Video'}
                    className="flex-1 py-3 bg-blue-600 text-white font-black uppercase tracking-widest rounded-2xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 text-sm flex-shrink-0"
                  >
                    {isLoading ? (lang === 'zh' ? '生成中...' : 'Generating...') : (lang === 'zh' ? '生成视频' : 'Generate Video')}
                  </button>
                </div>
              </div>
            )}
