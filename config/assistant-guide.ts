 ...
 * 
 * 为了降低token消耗，指南已按功能模块拆分
 * 系统会根据用户问题关键词自动选择相关模块
 */
...
- **重要提示**：多模态功能（视频分析、文件上传）主要通过神马API提供，其他API提供商的模型和功能会有所不同`,
    keywords: ['AI服务', '配置', 'API', '提供商', '密钥', '模型']
  },
  {
...
- **视频对话模式**：
  ...
  - 使用方式：输入提示词直接生成视频
  - 文件上传：不支持文件上传功能`,
    keywords: ['对话模式', '文本模式', '图像模式', '视频模式', '多模态', '文件上传']
  },
...
- **文档处理**：
  ...
  - 普通文本文件：直接读取文件内容
  - 支持中英文文档处理`,
    keywords: ['文件上传', '处理', '格式', '大小限制', '批量上传', '视频上传']
  },
...
- "曹操，删除选中的模块" - 删除操作
- "曹操，全选所有模块" - 选择所有模块`,
    keywords: ['语音', '语音控制', '语音识别', '唤醒词', '语音指令', '麦克风']
  },
  {
    name: 'voiceMore',
    content: `**视图控制**：
...
- 距离麦克风30-50厘米效果最佳
- 指令长度建议控制在10秒以内`,
    keywords: ['语音', '视图控制', '系统操作', '使用技巧', '自然语言']
  },
  {
    name: 'voiceClose',
    content: `### 语音控制关闭步骤
...
- **音频格式**：支持WebM、MP3、WAV等格式`,
    keywords: ['语音', '关闭步骤', '实时语音', '语音对话', '语音会话']
  },
  {
    name: 'realTimeVoice',
    content: `### 实时语音交互使用步骤
...
- **确认反馈**：重要操作请求AI确认，避免误操作`,
    keywords: ['实时语音', '语音对话', '语音模型', '语音交互', '会话', '音频']
  },
...
2. **使用手势操作**：
   ...
   - 系统识别成功后会执行相应操作并给出反馈
   - 可以连续使用不同手势，无需重复唤醒`,
    keywords: ['手势', '手势控制', '手势识别', '摄像头', 'AI分析', '学习能力']
  },
  {
    name: 'gestureOperations',
    content: `### 支持的手势操作
**缩放控制**：
...
### 手势控制关闭步骤
1. 点击左侧导航栏的"曹操"图标
2. 在手势控制面板中点击"手势控制"按钮
3. 系统提示"手势控制已关闭"
4. 此时您需要重新启用才能继续使用手势控制`,
    keywords: ['手势', '手势操作', '缩放', '移动', '画布操作', '选择操作', '关闭步骤']
  },
  {
    name: 'gestureTips',
    content: `### 手势使用技巧
...
3. **投射到画布**：
   ...
   - 新模块会自动排列在对应类型的区域`,
    keywords: ['投射', '内容投射', '投射到画布', '模块创建', '自动布局']
  },
  {
    name: 'projectionMore',
    content: `### 查看和编辑
...
- 系统资源使用统计
- 性能配置面板`,
    keywords: ['高级功能', '角色客串', '多图生成', '模板管理', '批量处理', '性能监控']
  },
  
  // 自动化 - 拆分为更小的子模块
  {
    name: 'automationBasic',
    content: `## 自动化控制面板
- 创建复杂的自动化工作流
- 支持条件触发和循环执行
- 批量数据输入和处理
...
5. 安全缓冲机制：确保结果保存完成后再开始下一轮`,
    keywords: ['自动化', '工作流', '批量处理', '执行机制', '节点', '依赖关系']
  },
  {
...
- 时间标记：\\{date\\} 生成的内容：\\{data\\}`,
    keywords: ['变量替换', '变量', '批量数据', '节点间', '时间变量', '模板']
  },
  {
    name: 'canvasAutoLayout',
    content: `## 画布模块自动排列系统
...
- 支持拖拽调整个别模块位置
- 自动避免模块重叠`,
    keywords: ['自动排列', '布局', '网格', '类型分区', '一键排序', '模块位置']
  },
...
- **状态指示**：显示当前选择的模型和可用状态
- **本地存储**：模型选择偏好自动保存`,
    keywords: ['画布', '界面', '主题', '语言', '操作', '预设提示词', '文本格式化', '视频编辑', '模型选择']
  },
  {
    name: 'accessibility',
    content: `## 无障碍功能
...
- **设置持久化**：所有无障碍设置自动保存到本地存储`,
    keywords: ['无障碍', '高对比度', '减少动画', '字体大小', '键盘导航', '屏幕阅读器']
  },
  {
    name: 'paintingEditing',
    content: `## 涂抹编辑和消失功能

### 涂抹编辑
- **技术**：使用火山引擎inpainting技术，支持图像局部编辑和修复
...
### 背景移除
- **技术**：使用ByteEdit技术自动抠图
- **效果**：生成透明背景图像，便于后续编辑和使用`,
    keywords: ['涂抹编辑', '涂抹消失', '背景移除', '画笔工具', '区域选择', '智能填充']
  },
  {
    name: 'exportDownload',
    content: `## 导出和下载
...
- 下载进度显示
- 断点续传支持`,
    keywords: ['导出', '下载', '单模块', '整体导出', '批量下载', 'ZIP']
  },
  {
    name: 'resourceLimits',
    content: `## 资源限制和性能优化
...
- 监控Token消费情况
- 定期重置性能统计`,
    keywords: ['资源限制', '性能优化', '内容限制', '文件限制', '性能限制', '优化技巧']
  },
  
  // 高级使用技巧 - 拆分为更小的子模块
  {
    name: 'tipsConversation',
    content: `## 高级使用技巧
...
- 预览功能可以帮助判断处理效果
- 对于重要图像，建议先备份原图`,
    keywords: ['对话模式', '涂抹编辑', '涂抹消失', '使用技巧', '多模态', '神马API']
  },
  {
    name: 'tipsAccessibility',
    content: `### 无障碍使用技巧
- 根据个人需求组合使用不同的无障碍功能
- 高对比度模式特别适合视觉障碍用户
- 减少动画功能有助于注意力集中
...
- 利用模块引用创建上下文
- 使用预设提示词提高效率`,
    keywords: ['无障碍', '模型选择', '提示词优化', '使用技巧', '多模态', '配置']
  },
  {
    name: 'tipsWorkflow',
    content: `### 工作流设计
...
- **资源管理**：大批量处理时注意Token消费和系统性能`,
    keywords: ['工作流', '自动化执行', '批量数据', '最佳实践', '依赖关系']
  },
  {
    name: 'tipsCanvas',
    content: `### 画布管理技巧
...
- 批量投射相关图片到画布
- 利用多图创建变体和选择`,
    keywords: ['画布管理', '模块管理', '批量生成', '角色客串', '多图生成', '视图控制']
  },
  
  // 故障排除 - 拆分为多个小模块
  {
    name: 'troubleshootingBasic',
    content: `## 故障排除和常见问题
...
- 检查模块引用是否正确
- 使用预设提示词作为参考`,
    keywords: ['故障排除', 'API配置', '视频上传', '生成质量', '常见问题', '解决方案']
  },
  {
    name: 'troubleshootingFiles',
    content: `## 故障排除 - 文件和连接问题
...
- 确认附件内容会自动包含，无需手动引用`,
    keywords: ['文件上传', '模块连接', '附件处理', '引用逻辑', '故障排除', '连接问题']
  },
  {
    name: 'troubleshootingFeatures',
    content: `## 故障排除 - 功能问题
...
- 检查模型选择是否正确保存
- 尝试切换不同模型进行测试`,
    keywords: ['涂抹编辑', '涂抹消失', '无障碍功能', '模型选择', '故障排除', '功能问题']
  },
  {
    name: 'troubleshootingInteraction',
    content: `## 故障排除 - 交互问题
...
- **语音延迟高**：与服务器距离和网络质量有关，可尝试切换网络环境`,
    keywords: ['手势控制', '语音控制', '实时语音', '交互问题', '故障排除', '识别失败']
  },
  {
    name: 'troubleshootingAutomation',
    content: `## 故障排除 - 自动化和画布问题
...
- **模块编号错乱**：模块编号按类型和创建顺序自动分配，无法手动修改`,
    keywords: ['自动化执行', '批量生成', '画布布局', '故障排除', '执行问题', '布局问题']
  },
  {
...
平台功能丰富且持续更新，建议结合实际使用场景逐步掌握各项功能。`,
    keywords: ['安全', '隐私', '数据安全', '隐私保护', '技术支持', '更新', '维护']
  }
];

// 生成完整的指南内容（用于向后兼容）
const generateFullGuideContent = (): string => {
  return guideModules.map(module => module.content).join('\n\n');
};

// 导出完整的指南内容常量（向后兼容）
export const ASSISTANT_GUIDE_CONTENT = generateFullGuideContent();
 ...
 * @returns 相关模块的内容字符串
 */
export const selectRelevantModules = (question: string): string => {
  // 将问题转换为小写以便匹配
  const lowerQuestion = question.toLowerCase();
  
  // 选择与问题相关的模块
  const relevantModules = guideModules.filter(module => 
    module.keywords.some(keyword => lowerQuestion.includes(keyword.toLowerCase()))
  );
  
  // 如果没有匹配到任何模块，返回核心模块
  if (relevantModules.length === 0) {
    const coreModule = guideModules.find(module => module.name === 'coreOverview');
    return coreModule ? coreModule.content : ASSISTANT_GUIDE_CONTENT;
  }
  
  // 确保核心模块总是包含在内（如果没有被自动选中）
  const hasCoreModule = relevantModules.some(module => module.name === 'coreOverview');
  const coreModule = guideModules.find(module => module.name === 'coreOverview');
  
  const finalModules = hasCoreModule 
    ? relevantModules 
    : [...(coreModule ? [coreModule] : []), ...relevantModules];
  
  // 合并选中的模块内容
  return finalModules.map(module => module.content).join('\n\n');
};

/**
 * 获取AI助手参考指南内容
 * @param question 可选的用户问题，用于选择相关模块
 * @returns 指南内容字符串
 */
export const getAssistantGuideContent = (question?: string): string => {
  // 如果提供了问题，根据问题选择相关模块
  if (question) {
    return selectRelevantModules(question);
  }
