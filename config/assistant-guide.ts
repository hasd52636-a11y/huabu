...
- **视频对话模式**：
  ...
  - 使用方式：输入提示词直接生成视频
  - 文件上传：不支持文件上传功能`,
    keywords: ['核心', '模块', '基础', '平台', '概述', '功能', 'AI服务', '配置', 'API', '对话模式', '文本模式', '图像模式', '视频模式', '多模态']
  },
  {
    name: 'modules',
    content: `## 基础操作流程
...
- **文档处理**：
  ...
  - 普通文本文件：直接读取文件内容
  - 支持中英文文档处理`,
    keywords: ['模块', '创建', '编辑', '连接', '引用', '附件', '文件上传', '处理', '格式', '大小限制', '批量上传']
  },
  {
    name: 'voice',
    content: `### 5. 语音控制系统 🎤
...
- **确认反馈**：重要操作请求AI确认，避免误操作`,
    keywords: ['语音', '语音控制', '语音识别', '唤醒词', '语音指令', '实时语音', '语音对话', '麦克风', '语音模型', '语音交互']
  },
  {
    name: 'gesture',
    content: `### 4. 手势控制系统 🤖
...
- **延迟高**：确保网络连接稳定，关闭其他占用摄像头的应用`,
    keywords: ['手势', '手势控制', '手势识别', '摄像头', '手势操作', '缩放', '移动', '画布操作', '选择操作']
  },
  {
...
- 错误日志和诊断
- 资源使用分析`,
    keywords: ['高级功能', '角色客串', '多图生成', '模板管理', '批量处理', '性能监控', '管理员', '监控面板']
  },
  {
    name: 'automation',
    content: `### 自动化控制面板
...
- 确保即使不使用变量语法，批量处理也能正常工作`,
    keywords: ['自动化', '工作流', '变量替换', '批量处理', '执行机制', '自动排列', '节点', '依赖关系']
  },
  {
    name: 'canvas',
    content: `## 用户界面功能
...
- 批量投射相关图片到画布
- 利用多图创建变体和选择`,
    keywords: ['画布', '界面', '主题', '语言', '操作', '预设提示词', '文本格式化', '视频编辑', '涂抹编辑', '无障碍', '模型选择', '导出', '下载', '资源限制', '性能优化', '使用技巧']
  },
  {
...
平台功能丰富且持续更新，建议结合实际使用场景逐步掌握各项功能。`,
    keywords: ['故障排除', '常见问题', 'API配置', '视频上传', '生成质量', '文件上传', '模块连接', '附件处理', '引用逻辑', '涂抹编辑', '手势控制', '语音控制', '自动化执行', '批量生成', '画布布局', '安全', '隐私', '技术支持', '更新', '维护']
  }
];
...
export const selectRelevantModules = (question: string): string => {
  ...
  const hasCoreModule = relevantModules.some(module => module.name === 'core');
  const coreModule = guideModules.find(module => module.name === 'core');
